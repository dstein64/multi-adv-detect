...
def run(params):
    ...
    with LOCK:
        # Randomly draw a model.
        model = np.random.choice(range(1, NUM_MODELS + 1))
        # Randomly draw a set of units.
        units = sorted(list(np.random.choice(range(REPRESENTATIONS_SIZE), size=params.num_units, replace=False)))
        ...
        with h5py.File(repr_path, 'r') as f:
            ...
            assert representations.shape[0] == REPRESENTATIONS_SIZE
            representations = representations[units].T
            ...
            representations = representations[np.where(params.eval_correct)[0]]
            assert type(representations) == np.ndarray
        adv_repr_lookup = {}
        for attack in ATTACKS:
            ...
            with h5py.File(adv_repr_path, 'r') as f:
                adv_representations = f['representations']
                assert adv_representations.shape[0] == REPRESENTATIONS_SIZE
                adv_representations = adv_representations[units].T
                assert representations.shape == adv_representations.shape
                assert type(adv_representations) == np.ndarray
            adv_repr_lookup[attack] = adv_representations
...
def main(argv=sys.argv):
    ...
    default_num_units_domain = list(takewhile(lambda x: x <= REPRESENTATIONS_SIZE, (2 ** x for x in count(3))))
    ...
    root_outdir = os.path.join(args.workspace, 'detect_unit_wise_control')
        ...
        for adv_target in ATTACKS:
            ...
            for num_units in args.num_units_domain:
                with open(os.path.join(outdir, f'{num_units}.csv'), 'w') as f:
                    f.write('model,units,correct,accuracy\n')
    params_list = []
    for num_units in args.num_units_domain:
